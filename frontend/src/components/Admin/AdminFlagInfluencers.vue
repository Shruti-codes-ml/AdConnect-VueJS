<template>
    <div class="p-4 bg-white bg-opacity-75 rounded shadow mt-3">
        <div v-if="errorMessage" class="alert alert-danger">
            {{ errorMessage }}
        </div>
        <h1 class="display-1">Hello @<span class="text-muted">{{ username }}</span></h1>
        
        <div class="d-flex justify-content-start gap-3">
            <button @click="goToSponsorApproval" class="btn btn-secondary"><i class="fa-solid fa-users-viewfinder"></i> Sponsor Approvals</button>
            <button @click="goToFlagInfluencer" class="btn btn-info"><i class="fa-solid fa-flag"></i> Flag Influencers</button>
            <button @click="goToFlagSponsor" class="btn btn-primary"><i class="fa-solid fa-flag"></i> Flag Sponsors</button>
            <button @click="goToFlagCampaign" class="btn btn-secondary"><i class="fa-solid fa-square-poll-horizontal"></i> Flag Campaigns</button>        
        </div>

        <h5 class="display-5">Manage Influencer Flags</h5>
        <table class="table table-hover">
            <thead>
                <tr>
                    <th>Username</th>
                    <th>Name</th>
                    <th>Category</th>
                    <th>Niche</th>
                    <th>Reach</th>
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="influencer in FlagInfluencers" :key="influencer.id">
                    <td>{{ influencer.username }}</td>
                    <td>{{ influencer.name }}</td>
                    <td>{{ influencer.category }}</td>
                    <td>{{ influencer.niche }}</td>
                    <td>{{ influencer.reach }}</td>
                    <td>
                        <button @click="toggleFlag(influencer)" :class="influencer.flagged ? 'btn btn-warning' : 'btn btn-info'">
                            <i class="fa-solid fa-flag"></i> {{ influencer.flagged ? 'Unflag' : 'Flag' }}
                        </button>
                    </td>
                </tr>
            </tbody>
        </table>
        <button @click="goToAdminHome" class="btn btn-primary mt-3"><i class="fa fa-arrow-circle-left"></i> Go Back</button>
    </div>
</template>

<script>
import axios from 'axios';

export default{
    data(){
        return{
            username:JSON.parse(localStorage.getItem('user')).username,
            FlagInfluencers : [],
            pendingInfluencerToHandle:null,
        };
    },
    created(){
        this.fetchInfluencers();
    },
    methods:{
        goToSponsorApproval() {
            this.$router.push('/admin/sponsor_approval');
        },
        goToFlagInfluencer(){
            this.$router.push('/admin/flag_influencers');
        },
        goToFlagSponsor(){
            this.$router.push('/admin/flag_sponsors');
        },
        goToFlagCampaign(){
            this.$router.push('/admin/flag_campaigns');
        },
        goToAdminHome() {
            this.$router.push('/admin_home');
        },
        fetchInfluencers() {
            const accessToken = localStorage.getItem('token');
            axios
                .get('/admin/flag_influencers', {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                    },
                })
                .then((response) => {
                    this.FlagInfluencers = response.data;
                })
                .catch((error) => {
                    console.error(error);
                });
        },  
        toggleFlag(influencer) {
            const accessToken = localStorage.getItem('token');
            const data = {
                influencer_id: influencer.id,
                flagged: !influencer.flagged,
            };
            axios
                .post('/admin/flag_influencers', data, {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                        'Content-Type': 'application/json',
                    },
                })
                .then((response) => {
                    console.log(response.data);
                    influencer.flagged = !influencer.flagged; // Update local state immediately
                })
                .catch((error) => {
                    console.error(error);
                });
        },
    }
}
</script>